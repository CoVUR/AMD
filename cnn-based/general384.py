...
callbacks = [
    ...
    SaveModelCallback(fname=name+'_384_'+str(i),monitor='f1_score'),
...
df = pd.read_csv('resultsDMAE/resultsTTA384.csv')
df = df.append(pd.DataFrame([resTTA], columns=['name','iteration','accuracy','precision','npv','recall','specificity','f1-score','AUROC','Cohen']))
df.to_csv('resultsDMAE/resultsTTA384.csv',index=None)


df = pd.read_csv('resultsDMAE/results384.csv')
...
df.to_csv('resultsDMAE/results384.csv',index=None)
